...
namespace TrinoClient
{
    ...
    internal class Query
    {
        ...
        public virtual IDictionary<string, ClientSelectedRole> SetRoles => _client.SetRoles;
        ...
        public async ValueTask<bool> MaterializeQueryOutput()
        {
            ...
            {
                var results = _client.Running ? _client.CurrentStatusInfo() : _client.FinalStatusInfo();
        ...
        {
            var status = results.UpdateType;
            if (results.UpdateCount != null)
            {
                var count = results.UpdateCount.Value;
